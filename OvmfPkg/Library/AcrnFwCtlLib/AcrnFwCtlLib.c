...
STATIC CONST CHAR8  mAcrnSig[4] = { 'A', 'C', 'R', 'N' };

STATIC BOOLEAN  mAcrnFwCtlSupported = FALSE;

STATIC INT32  mAcrnFwCtlTxid = 0xa5;
  ...
  )
{
  ...
  Status = AcrnFwCtl_CvtErr (Rhdr->Err);
  ...
  )
{
  ...
  AcrnFwCtl_MsgSend (OpCode, Sdata);
  Status = AcrnFwCtl_MsgRecv (&Rh, Rdata);
  ...
  )
{
  ...
  Status = AcrnFwCtl_Msg (OP_GET_LEN, Req, Resp);
  ...
  )
{
  ...
  Status = AcrnFwCtl_Msg (OP_GET, Req, Resp);
  ...
  )
{
  ...
  if (mAcrnFwCtlSupported == FALSE) {
    return RETURN_UNSUPPORTED;
  }

  if (Item == NULL) {
    Status = AcrnFwCtlGetLen (Name, (UINT32 *)Size);
  } else {
    Status = AcrnFwCtlGetVal (Name, Item, (UINT32 *)Size);
  }
