  ...
  )
{
  ...
  if ((Pci.Hdr.VendorId == 0xFB5D) && (Pci.Hdr.DeviceId == 0x40FB)) {
    DEBUG ((DEBUG_INFO, "ACRN framebuffer device detected\n"));
    Status = EFI_SUCCESS;

    AcrnGetGraphicsMode (PciIo, &Width, &Height, &Depth);
  ...
  )
{
  ...
  {
    DEBUG ((DEBUG_INFO, "ACRN GOP: No mmio bar\n"));
  } else {
    DEBUG ((
      DEBUG_INFO,
      "ACRN GOP: Using mmio bar @ 0x%lx\n",
      MmioDesc->AddrRangeMin
      ));
    AcrnGetMemregs (Private, &Memregs);
  ...
  if (EFI_ERROR (Status) ||
      (MmioDesc->ResType != ACPI_ADDRESS_SPACE_TYPE_MEM))
  {
    DEBUG ((DEBUG_INFO, "ACRN GOP: No frame-buffer bar\n"));
  } else {
    DEBUG ((
      DEBUG_INFO,
      "ACRN GOP: Using frame-buffer bar @ 0x%lx\n",
      MmioDesc->AddrRangeMin
      ));
  ...
  DEBUG ((
    DEBUG_INFO,
    "ACRN GOP: Framebuf addr 0x%lx, size %x\n",
    Private->FbAddr,
    Private->FbSize
    ));
  ...
  DEBUG ((DEBUG_INFO, "ACRN framebuffer device started\n"));
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "ACRN framebuffer device stopping\n"));
...
{
  ACRN_FBUF_MEMREGS  AcrnRegs;
  UINT64              Offset;
  EFI_STATUS          Status;

  Offset = (UINT64)&AcrnRegs.Width - (UINT64)&AcrnRegs;
  ...
  *Width  = AcrnRegs.Width;
  *Height = AcrnRegs.Height;
  *Depth  = AcrnRegs.Depth;

  DEBUG ((DEBUG_INFO, "ACRN Get Graphics Mode: w %d, h %d\n", *Width, *Height));
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "ACRN Set Graphics Mode: w %d, h %d\n", Width, Height));

  AcrnRegs.Width  = Width;
  AcrnRegs.Height = Height;
  AcrnRegs.Depth  = Depth;
  Offset           = (UINT64)&AcrnRegs.Width - (UINT64)&AcrnRegs;
  ...
  )
{
  ...
  DEBUG ((
    DEBUG_INFO,
    "ACRN Get Memregs, size %d width %d height %d\n",
    Memregs->FbSize,
    Memregs->Width,
    Memregs->Height
    ));
