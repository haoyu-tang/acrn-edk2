  ...
  )
{
  ...
  *SizeOfInfo = sizeof (EFI_GRAPHICS_OUTPUT_MODE_INFORMATION);
  ...
  (*Info)->PixelFormat          = PixelBitMask;
  (*Info)->PixelsPerScanLine    = (*Info)->HorizontalResolution;
  AcrnGopCompleteModeInfo (ModeData, *Info);
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  if (ModeNumber >= This->Mode->MaxMode) {
    // Tell acrn that we are switching out of vesa
    AcrnSetGraphicsMode (Private, 0, 0, 0);
    return EFI_UNSUPPORTED;
  }

  DEBUG ((DEBUG_INFO, "ACRN GopSetMode %d\n", ModeNumber));
  ...
  */
  ...
  DEBUG ((DEBUG_INFO, "ACRN GOP FrameBufferBase: 0x%x, FrameBufferSize: 0x%x\n", This->Mode->FrameBufferBase, This->Mode->FrameBufferSize));

  AcrnSetGraphicsMode (Private, (UINT16)ModeData->HorizontalResolution, (UINT16)ModeData->VerticalResolution, (UINT16)ModeData->ColorDepth);
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "ACRN Gop Constructor\n"));
