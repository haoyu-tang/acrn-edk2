  ...
  )
{
  ...
  if (QemuFwCfgIsAvailable ()) {
    if (EFI_ERROR (QemuFwCfgFindFile ("opt/acrn/hw.ncpu", &Item, &Size))) {
  ...
  )
{
  ...
  Status = AcrnGetCpuCount (&CpuCount);
  if (!EFI_ERROR (Status)) {
  ...
  )
{
  ...
  switch (Hdr->Signature) {
    case EFI_ACPI_1_0_APIC_SIGNATURE:
      TableInstallFunction = AcrnInstallAcpiMadtTable;
      break;
    default:
