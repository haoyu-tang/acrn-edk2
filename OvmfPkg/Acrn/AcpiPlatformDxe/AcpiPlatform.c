...
#define ACRN_ACPI_PHYSICAL_ADDRESS  ((UINTN)0x000F2400)
#define ACRN_BIOS_PHYSICAL_END      ((UINTN)0x00100000)
...
/**
  ...
  sources for the FV based tables is located under OvmfPkg/Acrn/AcpiTables.
  ...
  )
{
  ...
  TableInstallFunction = AcrnInstallAcpiTable;
  ...
  )
{
  ...
  Status = GetAcpiRsdpFromMemory (
             ACRN_ACPI_PHYSICAL_ADDRESS,
             ACRN_BIOS_PHYSICAL_END,
             &Rsdp
             );
  if (!EFI_ERROR (Status)) {
  ...
  if (EFI_ERROR (Status)) {
    DEBUG (
      (
       DEBUG_WARN,
       "%a: unable to install acrn's ACPI tables (%r)\n",
       __func__,
       Status
      )
      );
